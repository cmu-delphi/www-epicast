...
function debugForecasts(&$output, $regionID, $epiweek_now, $epiweek, $regionOrHosp) {
  if ($regionOrHosp == "regions") {
    $Users = mysql_query("select `user_id` from ec_fluv_forecast where `epiweek_now` = {$epiweek_now} and `region_id` = {$regionID} and `epiweek` = {$epiweek}");
  }
  else {
    $Users = mysql_query("select `user_id` from ec_fluv_forecast_hosp where `epiweek_now` = {$epiweek_now} and `group_id` = {$regionID} and `epiweek` = {$epiweek}");
  }
   $allUsers = array();
  ...
  $allWeeks = array();
  $weeks = mysql_query("select `epiweek` from ec_fluv_forecast where `epiweek_now` = {$epiweek_now} and `region_id` = {$regionID} and `user_id` = 1");
  while($row = mysql_fetch_array($weeks)) {
    // print ($row['epiweek']);
   ...
   $fourWeekAhead = array();
   if ($regionOrHosp == "regions") {
     $monthAhead = mysql_query("select `wili` from ec_fluv_forecast where `epiweek_now` = {$epiweek_now} and `region_id` = {$regionID} and `epiweek` = {$epiweek}");
     while($row = mysql_fetch_array($monthAhead)) {
          array_push($fourWeekAhead, $row['wili']);
     }
   }
   else {
     $monthAhead = mysql_query("select `value` from ec_fluv_forecast_hosp where `epiweek_now` = {$epiweek_now} and `group_id` = {$regionID} and `epiweek` = {$epiweek}");
     while($row = mysql_fetch_array($monthAhead)) {
   ...
   foreach ($allUsers as $u) {
     ...
     if ($regionOrHosp == "regions") {
       $allSeasonForecast = mysql_query("select `wili` from ec_fluv_forecast where `epiweek_now` = {$epiweek_now} and `region_id` = {$regionID} and `user_id` = {$u}");
       while ($row = mysql_fetch_array($allSeasonForecast)) {
         array_push($allForecasts[$u], $row['wili']);
       }
     } else {
       $allSeasonForecast = mysql_query("select `value` from ec_fluv_forecast_hosp where `epiweek_now` = {$epiweek_now} and `group_id` = {$regionID} and `user_id` = {$u}");
       while ($row = mysql_fetch_array($allSeasonForecast)) {
     ...
     foreach ($allWeeks as $week) {
       if ($regionOrHosp == "regions") {
         $forecast = mysql_query("select `epiweek`, `wili` from ec_fluv_forecast where `epiweek_now` = {$epiweek_now} and `region_id` = {$regionID} and `user_id` = {$u} and `epiweek` = {$week}");
         $row = mysql_fetch_array($forecast);
         ...
         $forecast = mysql_query("select `epiweek`, `value` from ec_fluv_forecast_hosp where `epiweek_now` = {$epiweek_now} and `group_id` = {$regionID} and `user_id` = {$u} and `epiweek` = {$week}");
         $row = mysql_fetch_array($forecast);
